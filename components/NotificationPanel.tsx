...
import { MegaphoneIcon, FileWarningIcon, ShieldCheckIcon, XCircleIcon, InboxIcon, BellIcon, AlertTriangleIcon, CheckCircleIcon, PencilIcon, TrendingDownIcon, ClipboardListIcon, ClipboardCheckIcon, ChartBarIcon, SyncIcon, CalendarIcon, UsersIcon } from './Icons';
...
const NOTIFICATION_ICONS: Record<NotificationType, { icon: React.ReactNode; color: string }> = {
  ...
  [NotificationType.LOW_PERFORMANCE_WARNING]: { icon: <TrendingDownIcon className="w-5 h-5" />, color: 'text-red-500' },
  [NotificationType.NEW_ASSIGNMENT]: { icon: <ClipboardListIcon className="w-5 h-5" />, color: 'text-cyan-500' },
  [NotificationType.PRECEPTOR_ATTENDANCE_TAKEN]: { icon: <ClipboardCheckIcon className="w-5 h-5" />, color: 'text-indigo-500' },
  [NotificationType.TEACHER_ATTENDANCE_TAKEN]: { icon: <ClipboardCheckIcon className="w-5 h-5" />, color: 'text-teal-500' },
  [NotificationType.NEW_GRADE]: { icon: <ChartBarIcon className="w-5 h-5" />, color: 'text-purple-500' },
  [NotificationType.SYNCED_ATTENDANCE_TAKEN]: { icon: <SyncIcon className="w-5 h-5" />, color: 'text-cyan-500' },
  [NotificationType.UPCOMING_EVENT]: { icon: <CalendarIcon className="w-5 h-5" />, color: 'text-blue-500' },
  [NotificationType.NEW_EVENT_PARTICIPANT]: { icon: <UsersIcon className="w-5 h-5" />, color: 'text-teal-500' },
  // New ones
  [NotificationType.TASK_COMPLETED]: { icon: <CheckCircleIcon className="w-5 h-5" />, color: 'text-green-500' },
  [NotificationType.NEW_INCIDENT]: { icon: <AlertTriangleIcon className="w-5 h-5" />, color: 'text-red-500' },
  [NotificationType.INCIDENT_RESOLVED]: { icon: <ShieldCheckIcon className="w-5 h-5" />, color: 'text-green-500' },
  [NotificationType.SHIFT_CHANGE_REQUEST]: { icon: <UsersIcon className="w-5 h-5" />, color: 'text-purple-500' },
  [NotificationType.SHIFT_CHANGE_ACCEPTED]: { icon: <CheckCircleIcon className="w-5 h-5" />, color: 'text-green-500' },
  [NotificationType.SHIFT_CHANGE_REJECTED]: { icon: <XCircleIcon className="w-5 h-5" />, color: 'text-red-500' },
};
