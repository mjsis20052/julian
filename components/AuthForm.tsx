...
export const AuthForm: React.FC<AuthFormProps> = ({ onLogin, onRegister }) => {
  ...
  const handlePrototypeLogin = (userId: number, roleName: string) => {
    const user = INITIAL_USERS.find(u => u.id === userId);
    if (user) onLogin(user);
    else triggerError(`No se encontró el usuario prototipo de ${roleName}.`);
  };
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    try {
      const user = INITIAL_USERS.find(u => {
          if ((u.email.toLowerCase() !== email.toLowerCase() && u.name.toLowerCase() !== email.toLowerCase()) || u.password !== password || u.role !== role) {
              return false;
          }
          const requiresCareerYear = [Role.STUDENT, Role.PRECEPTOR, Role.TEACHER, Role.STUDENT_REP].includes(role);
          // FIX: Handle user year being an array when comparing with the selected year.
          if (requiresCareerYear && (u.careerId !== careerId || (Array.isArray(u.year) ? !u.year.includes(year) : u.year !== year))) {
              return false;
          }
          return true;
      });

      if (!user) throw new Error("Credenciales incorrectas. Verifique los datos ingresados.");
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      const newUser: User = {
          ...
          password,
          role,
          careerId: [Role.STUDENT, Role.PRECEPTOR, Role.TEACHER, Role.STUDENT_REP].includes(role) ? careerId : undefined,
          year: [Role.STUDENT, Role.PRECEPTOR, Role.TEACHER, Role.STUDENT_REP].includes(role) ? year : undefined,
      };
      onRegister(newUser);
    } catch (err: any) {
      triggerError(err.message);
  ...
  const showCareerYearFields = [Role.STUDENT, Role.PRECEPTOR, Role.TEACHER, Role.STUDENT_REP].includes(role);

  const renderSelectionScreen = () => (
     <div className="animate-fade-in-up">
        <div className="prototype-buttons grid grid-cols-2 gap-3 animate-slide-in-up" style={{ animationDelay: '0.3s' }}>
            <button type="button" onClick={() => handlePrototypeLogin(101, 'alumno')} className="prototype-button !py-3"><UserIcon className="w-5 h-5"/><span>Alumno</span></button>
            <button type="button" onClick={() => handlePrototypeLogin(1, 'preceptor')} className="prototype-button !py-3"><ShieldCheckIcon className="w-5 h-5"/><span>Preceptor</span></button>
            <button type="button" onClick={() => handlePrototypeLogin(601, 'profesor')} className="prototype-button !py-3"><UserIcon className="w-5 h-5"/><span>Profesor</span></button>
            <button type="button" onClick={() => handlePrototypeLogin(701, 'auxiliar')} className="prototype-button !py-3"><UserIcon className="w-5 h-5"/><span>Auxiliar</span></button>
            <button type="button" onClick={() => handlePrototypeLogin(801, 'c.e.')} className="prototype-button !py-3 col-span-2"><UserIcon className="w-5 h-5"/><span>C. Estudiantes</span></button>
        </div>
    ...
    <form onSubmit={handleLoginSubmit} className="space-y-5">
      ...
      <div className="input-group animate-slide-in-up" style={{ animationDelay: '0.5s' }}>
          <select value={role} onChange={(e) => setRole(e.target.value as Role)} className="input-field">
              ...
              <option value={Role.STAFF}>Auxiliar</option>
              <option value={Role.STUDENT_REP}>Centro de Estudiantes</option>
          </select>
      </div>
      {showCareerYearFields && (
        <>
