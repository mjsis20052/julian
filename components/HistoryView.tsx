...
export const HistoryView: React.FC<HistoryViewProps> = ({ user, students, records, subjects }) => {
  ...
  const [filterYear, setFilterYear] = useState((Array.isArray(user.year) ? user.year[0] : user.year) || 1);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records
      .filter(record => {
        ...
        if (!student || student.careerId !== user.careerId || (Array.isArray(student.year) ? !student.year.includes(filterYear) : student.year !== filterYear)) return false;
